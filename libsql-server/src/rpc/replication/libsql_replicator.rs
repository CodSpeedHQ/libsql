...
impl LibsqlReplicationService {
    ...
    async fn authenticate<T>(
        ...
        super::auth::authenticate(&self.store, req, namespace, &self.user_auth_strategy).await
    }
            ...
            Some(Ok(f)) => {
                match this.flavor {
                    WalFlavor::Libsql => {
                        ...
                        let bytes: Box<[u8; size_of::<Frame>()]> = unsafe { std::mem::transmute(f) };
...
impl ReplicationLog for LibsqlReplicationService {
    ...
    async fn snapshot(
        ...
        Err(Status::unimplemented("no snapshot required with libsql wal"))
    }
}
