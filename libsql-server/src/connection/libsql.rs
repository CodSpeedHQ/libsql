...
impl MakeConnection for MakeLibsqlConnection {
    ...
    async fn create(&self) -> crate::Result<Self::Connection> {
        ...
        let core = BLOCKING_RT.spawn_blocking(move || -> crate::Result<_> {
        ...
        Ok(LibsqlConnection { inner: Arc::new(Mutex::new(core)) })
    }
}
...
impl LibsqlConnection {
    pub async fn execute<B: QueryResultBuilder>(
        ...
        check_program_auth(&ctx, &pgm, &self.inner.lock().config())?;
