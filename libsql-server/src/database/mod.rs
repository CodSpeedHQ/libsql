...
pub use self::libsql_primary::{LibsqlPrimaryConnection, LibsqlPrimaryDatabase, LibsqlPrimaryConnectionMaker};
...
impl Database {
    pub fn connection_maker(&self) -> Arc<dyn MakeConnection<Connection = Connection>> {
        match self {
            ...
            Database::LibsqlPrimary(db) => Arc::new(db.connection_maker().map(Connection::LibsqlPrimary)),
            Database::LibsqlReplica(db) => Arc::new(db.connection_maker().map(Connection::LibsqlReplica)),
            Database::LibsqlSchema(db) => Arc::new(db.connection_maker().map(Connection::LibsqlSchema)),
        }
    }
    ...
    pub fn notifier(&self) -> Option<watch::Receiver<Option<FrameNo>>> {
        match self {
            ...
            Database::LibsqlPrimary(_) => todo!(),
            Database::LibsqlReplica(_) => todo!(),
            Database::LibsqlSchema(_) => todo!(),
        }
    }
